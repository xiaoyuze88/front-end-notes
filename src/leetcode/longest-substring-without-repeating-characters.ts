...
function lengthOfLongestSubstringDP(s: string): number {
  ...
  for (let i = 1; i < s.length; i++) {
    prefix = s.substring(i - lastCount, i);

    // console.log(s[i]);

    const lastIndex = prefix.lastIndexOf(s[i]);

    if (lastIndex !== -1) {
      // lastCount = i - (i - lastCount + lastIndex);
      lastCount = lastCount - lastIndex;
    } else {
...
printResult(lengthOfLongestSubstringDP, ["abcabcbb"], 3);
printResult(lengthOfLongestSubstringDP, ["bbbbb"], 1);
printResult(lengthOfLongestSubstringDP, ["pwwkew"], 3);
printResult(lengthOfLongestSubstringDP, [" "], 1);
printResult(lengthOfLongestSubstringDP, [""], 0);
printResult(lengthOfLongestSubstringDP, ["dvdf"], 3);
printResult(lengthOfLongestSubstringDP, ["qrsvbspk"], 5);
printResult(lengthOfLongestSubstringDP, ["aab"], 2);
printResult(lengthOfLongestSubstringDP, ["abba"], 2);
